...
describe("AttachmentWorkspace", () => {
  ...
  it("offers an explicit recovery mode only when legacy edits are available", async () => {
    const user = userEvent.setup();
    const onOpenLegacyRecovery = vi.fn();
    const services: AttachmentWorkspaceServices = {
      inspect: vi.fn().mockResolvedValue({
        documentType: "pdf",
    ...
    render(
      <NextIntlClientProvider locale="en" messages={en} timeZone="UTC">
        <AttachmentWorkspace
          file={pdfFile}
          services={services}
          onOpenLegacyRecovery={onOpenLegacyRecovery}
        />
      </NextIntlClientProvider>
    );

    expect(await screen.findByText("Legacy doXmind edits found")).toBeInTheDocument();
    await user.click(screen.getByRole("button", { name: "Open legacy recovery" }));
    expect(onOpenLegacyRecovery).toHaveBeenCalledOnce();
  });
});
