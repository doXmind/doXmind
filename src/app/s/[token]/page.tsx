...
import { InlineCommentsSidebar } from "@/components/shared/inline-comments-sidebar";
import { InlineCommentsMobile } from "@/components/shared/inline-comments-mobile";
...
export default function SharePage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        const sharedDoc = await api.getSharedDocument(token, path || undefined);
        setData(sharedDoc);

        // Try to load community detail for richer metadata (may fail for private shares)
        try {
  ...
  // ──────────────────────────────────────────────────────────────────────────
  // Unified share view — adapts UI based on visibility
  // ──────────────────────────────────────────────────────────────────────────
  ...
  return (
    <AppShell>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="border-b border-border/40">
            <div className="mx-auto max-w-3xl px-6 pb-10 pt-10 sm:px-8 lg:max-w-5xl">
              {/* Top row: back link / private badge + reading toolbar */}
              ...
              {/* Tags (public only) */}
              {isPublic && detail && detail.tags.length > 0 && (
              ...
              {detail && (
                <div ref={actionBarRef} className="mt-8 flex flex-wrap items-center gap-3">
                  ...
                  />
                  {isPublic && currentUser?.id === detail.owner.id && (
                    <button
                ...
                {data.is_folder ? (
                  <>
                    ...
                    {folderLoading ? (
                      ...
                      <SharedDocumentView data={folderViewData} embedded shareToken={token} />
                    ) : (
                      <FolderItemsList
                  ...
                  <SharedDocumentView data={data} embedded shareToken={token} />
                )}
              </article>
      ...
      {/* Inline comments — private shares only */}
      {!isPublic && (
        <>
          <InlineCommentsSidebar shareToken={token} />
          <InlineCommentsMobile shareToken={token} />
        </>
      )}

      {/* Edit share metadata modal (public, owner only) */}
