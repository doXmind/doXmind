...
export function InlineErrorBanner() {
  ...
  return (
    <div
      ...
      // Anchored below the chrome band, not on top of it. At top-3 the card
      // covered y=12..56, i.e. the whole 32px-tall tab strip, so a rename
      // conflict took the tabs away for the five seconds you most wanted them.
      // The band ends at y=80 (44px header + the 36px Page-controls row), so
      // 88px clears it with the same 8px gap the panels use.
      className="pointer-events-none fixed left-1/2 top-[88px] z-[1000] flex w-full max-w-[480px] -translate-x-1/2 flex-col gap-1.5 px-3"
    >
...
function ErrorBannerRow({ error }: ErrorBannerRowProps) {
  ...
  return (
    <motion.div
      ...
      layout
      // Only the dismiss control takes pointer events. A transient card that
      // swallows clicks over a 480x44 rectangle is a trap whatever it sits on.
      className="pointer-events-none flex items-start gap-2.5 rounded-lg border border-destructive/25 bg-popover px-3.5 py-2.5 text-popover-foreground shadow-[0_1px_0_rgba(15,15,15,0.02),0_4px_16px_rgba(15,15,15,0.06)]"
    >
      ...
      <button
        ...
        className="pointer-events-auto -mr-1 -mt-0.5 flex size-6 flex-shrink-0 cursor-pointer items-center justify-center rounded text-foreground/45 transition-colors hover:bg-foreground/[0.06] hover:text-foreground/80"
      >
